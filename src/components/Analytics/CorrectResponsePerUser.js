...
import {defaultLayout, defaultSettings, hoverData} from "../../utils/plotUtils";
...
const CorrectResponsePerUser = ({maxWidth}) => {
  ...
  const chartData = Array.from(
      responses?.groupBy((r) => r.memberId)
  ).reduce(
      (acc, [id, list], idx) => {

        const nbCorrect = list.reduce((acc, next) =>
          computeCorrectness(next.data, questionsById?.get(next.data.questionId)?.first()?.data)?
              acc + 1 : acc
        , 0)
